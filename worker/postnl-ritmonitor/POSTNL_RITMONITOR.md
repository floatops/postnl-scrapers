...
- **Blokkade herkennen** (`detecteerBlok()`, ook uit stap1, sinds #173 in `credentials-shared/src/proxyPool.js`): bij een loginveld dat niet zichtbaar wordt, wordt de paginatitel/-inhoud gecontroleerd op Akamai-/WAF-patronen en als `⛔ MOGELIJKE IP-BLOKKERING` gelogd. Zonder dit staat er in `worker_run_log` alleen een kale time-out, en is "PostNL-hik" niet te onderscheiden van "ons IP ligt eruit".
- **Met de proxy-pool (VPS, `PROXY_POOL=true`, #173)** gaat dit verder: een blokkade (herkend op de loginpagina, een HTTP 403, of een mislukte lezing waarna de pagina een blokkade toont) sluit het IP af als `blokkade` in `proxy_gebruik_log` — de database zet het in quarantaine en meldt het — en de sessie wordt direct vervangen door een met het volgende IP, zonder eerst op hetzelfde IP opnieuw te proberen. Elke sessie krijgt een eigen IP; `storageState` wordt dan niet hergebruikt (elke run vers inloggen, zoals op GitHub).
